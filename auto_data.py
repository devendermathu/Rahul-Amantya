...
import csv
s = Session()
f = open('after_14130_data.csv',mode='w',encoding='utf-8',newline="")
fr = open('all_links.txt',mode='r',encoding='utf-8')
...
for i in rd[14130:]:
    try:
        ...
        kil = soup.find('span','sc-font-l cldt-stage-primary-keyfact')
        if(kil):
            kl = soup.find('span','sc-font-l cldt-stage-primary-keyfact').text.strip()+'|'
        else:
            kl = 'Not given'
        ...
        # if(grid):
        #     dt = [i1.text.strip().replace('\n','') for i1 in grid.find_all('dt')]
        #     dd = [i2.text.strip().replace('\n','') for i2 in grid.find_all('dd')]
        #     for d1,d2 in zip(dt,dd):
        #         n4 += '{}:{}|'.format(d1,d2)
        tb = soup.find('div','cldt-data-section sc-grid-col-s-12')
        if(tb):
            fuel = tb.find('a').text
        else:
            fuel = 'Not given'
